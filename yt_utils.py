...
def get_metadata(url: str):
    ...
    """
    result = subprocess.run(f"youtube-dl -j {url} --cookies ytcookies.txt", capture_output=True)
    ic(f"[command] youtube-dl -j {url} --cookies ytcookies.txt")
    return result
...
def get_youtube_meta_data(video_url: str)-> List[dict]:
     with youtube_dl.YoutubeDL({'outtmpl': '%(id)s.%(ext)s', 'cookiefile': 'ytcookies.txt'}) as ydl:
